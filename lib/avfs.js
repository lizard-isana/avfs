...
AVfS.GetCurrentMesh = function(position, mesh){

  var lat_key = function(){
    if(position.latitude>0){
      var prefix = "N";
    }else{
      var prefix = "S";
    }
    return prefix + zero_padding(Math.floor(Math.abs(position.latitude)/30)*30,3);
  }
 
  var lng_key = function(){
    var num = Math.floor(Math.abs(position.longitude)/30)*30;

    if(position.longitude>=0 && position.longitude<15){
      var prefix = "W";      
      num = num+30;
    ...
    if(["N060","S000","S060"].indexOf(lat_key())<0){
      return prefix + zero_padding(num,3);
    }else{
      return prefix + zero_padding(num+15,3);
    }
  }
 
  function zero_padding(num,len){
    var n = String(num);
    if(n.length == len){return num}
    var str = "";
    for(var i = 0; i<len; i++){
      str = "0" + String(str)
      if((str+n).length>len-1){
        return str + n;
      }
    }
  }

  var latitude_key = lat_key()
  var longitude_key = lng_key()
  var current_mesh = mesh[latitude_key][longitude_key];
  return current_mesh
...
AVfS.ShowLabels = function(center){
  ...
  var city = [
    ["San Francisco",37.790455,-122.416534],
    ["Boulder",40.016178,-105.272799],
    ["Miami",25.778561,-80.379324],
    ["New York",40.741079,-73.994894],
    ["Vancouver",49.288803,-123.12026],
    ["Montreal",45.485891,-73.595695],
    ["Antartica/McMurdo Station",-77.837726,166.68663],
    ["Sao Paulo",-23.521339,-46.655416],
    ["Santo Domingo",18.451685,-69.662633],
    ["Santiago",-33.493844,-70.603251],
    ["Nairobi",37.0625,-95.677068],
    ["Tokyo",35.908309,139.93475],
    ["Jakarta",-6.221641,106.809597],
    ["Bangalore",12.976624,77.631111],
    ["Tel Aviv",32.083455,34.790471],
    ["Exeter",50.727281,-3.475134],
    ["Oxford",51.635919,-1.332092],
    ["Dublin",53.341527,-6.268827],
    ["Lausanne",46.541506,6.539955],
    ["Stuttgart",48.783455,9.223022],
    ["Istanbul",41.082424,29.001932],
    ["Melbourne",-37.702022,144.948807],
    ["Adelaide",-34.921056,138.592186],
    ["Sydney",-33.898489,151.232986],
    ["Canberra",-35.320938,149.009221]
  ]
