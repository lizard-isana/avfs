...
function removeScript(id){
  var ScriptObject=document.getElementById(id);
  var objHead = document.getElementsByTagName("head")[0]; 
  objHead.removeChild(ScriptObject);
}
...
function registerGeoLocatedTwitterers(data){
    ...
    for(var i = 0, dl = data.length ; i < dl; ++i){
        var now = new Date()
        if(((now.getTime() - Date.parse(data[i].created_at))/(60*60*1000))<24){
...
function updateTweets(){
    ...
    if(document.getElementById("tweet_jsonp")){
     removeScript("tweet_jsonp")
    }
    addScript(url, "tweet_jsonp");
}
